...
int init()
{
  ...
  textureId = loadTexture("../textures/Graffiti0111_S.jpg"); 
  textureId2 = loadTexture("../textures/TropicalFoliage0025_1_S.jpg"); 
  setModelCenter(model, 0.f, 0.f, 0.f);
  setModelRadius(model, 0.3f);

  // Create vertex shader
  GLuint vertexShader = createShaderFromFile(GL_VERTEX_SHADER, "lab3-8.vs");
  if (!vertexShader)
    return 0;

  // Create fragment shader
  GLuint fragmentShader = createShaderFromFile(GL_FRAGMENT_SHADER, "lab3-8.fs"); 
  if (!fragmentShader)
    return 0;
  ...
  shaderTextureLocation2 = getUniformLocation(shaderProgram, "texture2");
  return 1;
}
