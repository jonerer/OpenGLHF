...
int init()
{
  ...
  textureId2 = loadTexture("../textures/CrackedEarth0010_1_S.jpg"); 
  setModelCenter(model, 0.f, 0.f, 0.f);
  setModelRadius(model, 0.3f);

  // Create vertex shader
  GLuint vertexShader = createShaderFromFile(GL_VERTEX_SHADER, "lab3-7.vs");
  if (!vertexShader)
    return 0;

  // Create fragment shader
  GLuint fragmentShader = createShaderFromFile(GL_FRAGMENT_SHADER, "lab3-7.fs"); 
  if (!fragmentShader)
    return 0;
